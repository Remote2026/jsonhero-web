    ...
    <>
            ...
            {searchState.status !== "idle" &&
              (!searchState.results || searchState.results.length === 0) && (
                <div className="results-loading flex">
                  ...
                  <Body className="text-slate-400">{t("search.loading")}</Body>
                </div>
              )}
            {searchState.results && searchState.results.length > 0 && (
              <div className="results-returned">
                <Body className="text-slate-400">
                  {searchState.results.length === 1
                    ? t("search.oneResult")
                    : t("search.nResults", { count: searchState.results.length })}
                </Body>
              </div>
            )}
            {searchState.status === "idle" &&
              ...
              (!searchState.results || searchState.results.length === 0) && (
                <div className="results-none flex">
                  ...
                  <Body className="text-slate-400">
                    {t("search.noResults", { query: cb.inputValue })}
                  </Body>
                </div>
              )}
          </div>
      ...
      <div className="flex items-center w-full gap-4 px-3 py-2 border-t-[1px] bg-slate-100 border-slate-200 rounded-br-lg rounded-bl-lg transition dark:bg-slate-900 dark:border-slate-700">
        <div className="flex items-center gap-1">
          ...
          <Body className="text-slate-700 dakr:text-slate-500">{t("search.toSelect")}</Body>
        </div>
        <div className="flex items-center gap-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <Body className="text-slate-700 dakr:text-slate-500">{t("search.toClose")}</Body>
        </div>
      </div>
    </>
  );
